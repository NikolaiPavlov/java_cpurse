    ...
    static <T extends Comparable<T>> List<T> sort(List<T> list) {
        ...
        do {
            ...
            for (int i = 0; i < list.size(); i += size_block) {
                ...
                for (int j = 0; (j < size_block) && (j + i < list.size()); j++) {// цикл по элементам блока (блок может быть не полным (конец списка))
                    if ((pos2 == i + size_block) || (pos2 >= list.size())) {// индекс 2й половины блока вышел за пределы => просто добавляем элементы из первой половины
                        newlist.add(list.get(pos1++));// добавляем элемент из 1й половины блока и увеличиваем индекс 1й половины
                        continue;
                    }
                    if (pos1 == i + (size_block >> 1)) {// индекс 1й половины блока вышел за пределы => просто добавляем элементы из 2й половины
                        newlist.add(list.get(pos2++));
                        continue;
                    }
                    if (list.get(pos1).compareTo(list.get(pos2)) < 0)// сравниваем элементы из 2х половин блока и берем наименьший, увеличивая индекс соответствующей половины блока
